...
v_{0:7}=(0.0497,\,0.0632,\,0.1108,\,0.1770,\,0.036,\,-1.496,\,-5.34,\,-21.5),
\]
...
Monte-Carlo-free ground truth to better than $2\%$ for $\beta\ge8$ and to $9\%$ at $\beta=4$, with
an error that rises \emph{monotonically} in $\eta^2$ beyond the ground-truth noise floor --- establishing that the representation is correct and
the approximant merely starved outside its disc. At $\beta=2$ the reconstruction is $36\%$ high and
remains open.
\end{abstract}
...
0.25 & 16.0 & 0.074659 & 0.0740 & $-0.8\%$ & 0.0736 & $-1.5\%$\\
0.49 &  8.2 & 0.104324 & 0.0859 & $-17.7\%$ & 0.1043 & $\mathbf{-0.0\%}$\\
0.81 &  4.9 & 0.133752 & 0.0753 & $-43.7\%$ & 0.1405 & $+5.1\%$\\
1.00 &  4.0 & 0.145188 & 0.0666 & $-54.1\%$ & \textbf{0.1583} & $\mathbf{+9.0\%}$\\
1.44 &  2.8 & 0.159783 & 0.0497 & $-68.9\%$ & 0.1913 & $+19.7\%$\\
2.00 &  2.0 & 0.164683 & 0.0354 & $-78.5\%$ & 0.2234 & $+35.6\%$\\
2.50 &  1.6 & 0.162948 & 0.0271 & $-83.4\%$ & 0.2470 & $+51.6\%$\\
\noalign{\smallskip}\hline
\end{tabular}
\end{center}
...
The eight-coefficient error rises \emph{monotonically} in $x$ for $x\ge0.36$, never erratic in sign or size. (Below $x=0.36$ it wiggles by under $1.5\%$, which is the accuracy of the ground-truth solver itself, so no structure should be read into it.) This is a
...
$x\le0.36$ ($\beta\ge11$) with seven --- and reconstructs $\Var$ to better than $2\%$ for
$\beta\ge8$. Adding $v_7$ also flips the $\beta=2$ error from $79\%$ low to $36\%$ high, so the truth
...
the weight. The top rung $v_7$ now rests on two usable grid points, $n=16,18$, which differ by only $1.8\%$; its band is $[-25.3,-21.5]$, tighter in relative terms than $v_6$'s.

\emph{Open.} (i) $\beta=2$ is not reconstructed ($36\%$). (ii) A ninth coefficient $v_8$ is
