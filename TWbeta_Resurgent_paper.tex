...
$\Omega_1=-1.1248139\ldots$ a finite renormalized Airy-Green's chaos integral, available in closed
form as two Airy-quartic moments (\S\ref{sec:stokes}, eq.~\eqref{eq:om1exact}).
...
\begin{lemma}[Noise rigidity: $\beta$ enters only the amplitude]\label{lem:rigidity}
Substitute $u=e^{-W/\eps^2}\sum_{n\ge0}\eps^{2n}A_n$ into the backward-Kolmogorov equation
\eqref{eq:bk} and collect powers of $\eps=\beta^{-1/2}$. The eikonal (order $\eps^{-2}$) and the
leading transport (order $\eps^{0}$) are \emph{$\eps$-free},
\[
-\partial_x W-b\,\partial_p W+2(\partial_p W)^2=0,\qquad
\bigl[\partial_x+(b-4\,\partial_p W)\,\partial_p\bigr]A_0=2\,(\partial_p^2 W)\,A_0,
\qquad b=(x+a)-p^2,
\]
the diffusion term $2\eps^2\partial_p^2$ contributing $2(\partial_p W)^2$ to the eikonal (its $\eps^2$
cancelling the $\eps^{-4}$ of $(\partial_p W/\eps^2)^2$), and the order-$\eps^0$ equation closing on
$A_0$ alone. The noise strength first enters at order $\eps^2$ (the equation for $A_1$). Hence the
level expansion \textbf{(L)}'s Borel-singularity \emph{locations} (instanton actions $=$ differences
of eikonal solutions $W$) and its \emph{index} $\gamma$ (fixed by the leading prefactor $A_0$) are
$\beta$-independent, and $\beta$ enters only the \emph{amplitude}: the loop factor
$\sum_{n\ge1}c_n(a)\beta^{-n}$, $c_n=A_n(+\infty,0)$. \tagPr
\end{lemma}
\begin{proof}
The three displayed statements are the $\eps^{-2},\eps^{0},\eps^{2}$ coefficients of \eqref{eq:bk}
under the ansatz (collected symbolically). The eikonal is the Hamilton--Jacobi equation for the
Freidlin--Wentzell quasipotential of the $1$-D small-noise diffusion $\dd p=b\,\dd x+2\eps\,\dd B$;
the quasipotential and its leading Ludwig prefactor $A_0$ are noise-amplitude independent, only the
higher WKB corrections carrying $\eps$. Since the exponential scales of the level resurgence are the
action differences and its index is set by $A_0$, both are $\beta$-independent. \qed
\end{proof}

\begin{remark}[The distributional potential plays no role]\label{rem:rough}
Lemma~\ref{lem:rigidity} is the rigorous content of the ``isomonodromy rigidity'' invoked for
location-fixing (\S\ref{sec:T2proof}): the white-noise potential of $\SAO$ never appears, having been
absorbed into the \emph{smooth} degenerate diffusion coefficient $2\eps^2\partial_p^2$ of
\eqref{eq:bk} by the Riccati/RRV reduction. What survives is the standard smooth exact-WKB fact that
the $\eps^{2n}$ transport hierarchy adds no new exponential scales --- the same Costin/exact-WKB input
already used in Theorem~\ref{thm:T1}, and no separate rough-potential estimate.
\end{remark}
...
saddle-vs-distribution distinction, not a $\beta$-dependence. The assignment is consistent, and this
can be checked directly: the $\alpha=\tfrac12$ saddle action gives
$\mathcal T_\beta(a)\sim a^{-3\beta/4}e^{-(2\beta/3)a^{3/2}}$ (Prop.~\ref{prop:anchor}), which at
$\beta=2$ reproduces the classical $\alpha=0$ Hastings--McLeod right tail
$e^{-(4/3)a^{3/2}}/(16\pi a^{3/2})$ \cite{TW1994} in both exponent and prefactor power---the ratio of
the two $\to1$ (e.g.\ $0.941,0.957,0.967$ at $a=8,10,12$, from a direct Hastings--McLeod solve). So
$\alpha=\tfrac12$ governs the saddle/rate and $\alpha=0$ the distribution; since Costin's class
contains PII for all $\alpha$, Theorem~\ref{thm:T1} holds regardless. \tagPr
\end{remark}
...
sector exactly; its extension to a statement uniform in $a$ at fixed finite $a\ge a_0$ is
Remark~\ref{rem:uniform}. \tagP
\end{remark}

\begin{remark}[Uniform in $a$: the two-directional closure]\label{rem:uniform}
Collecting the exponent of \eqref{eq:bn} in powers of $1/\beta$ gives the noise coefficients
$\sigma_n(a)=2^n\sum_{m\ge n}r_{m,n+1}\,a^{-3m/2}$, $r_{m,\ell}:=[X^\ell]R_m(X)$, $X=2/\beta$; the
$a$-independent prefactor $c_\beta$ (Coulomb-gas, $i\R$) and $a^{-3\beta/4}$ carry no $1/\beta$-Borel
content, so the noise resurgence is exactly $\{\sigma_n\}_{n\ge1}$. For $a\ge a_0$ this series is median
Borel summable along $\R_+$ \emph{uniformly in $a$}, on three legs. \emph{(i) Sector.} The dynamical
singularity is the noise-independent action $\Phi(a)=\tfrac23a^{3/2}\in\R_+$ (\S\ref{sec:T2proof}),
moving with $a$ but never leaving the ray, while the Coulomb-gas singularities are fixed on $i\R$; a
median sector of opening $>\pi$ avoiding $i\R$ therefore exists uniformly. \emph{(ii) Diagonal.} The
scaling coefficient $2^n r_{n,n+1}$ is the \emph{odd part} of $\log F$, $F=\tilde Q(2/g)^{-2}$ of
\eqref{eq:R} --- equivalently the median of the frozen escape $\log F(g)$ and its reflection
$-\log F(-g)$ (the genus-zero WKB branch), matched through the computable orders; the even part is the
Stokes discontinuity, whence the degree deficit of the $R_{2k}$. It inherits $F$'s Gevrey-$1$ bound,
$|2^n r_{n,n+1}|\le\tfrac1\pi\Gamma(n)(3/2)^n$. \emph{(iii) Amplitude.} The off-diagonal is the level
($a$) resurgence, a divergent Gevrey-$1$ series in $a^{-3/2}$ of Borel radius $\rho$ set by the level
action --- $\rho\approx\tfrac{2\sqrt2}3$ (the Hastings--McLeod value) at $\beta{=}2$, so
$a_0=\rho^{2/3}\approx1$; and since $\sigma_n(a)\sim\tfrac{S(a)}{2\pi i}\Gamma(n)\Phi(a)^{-n}$, its
Gevrey constant is the noise Stokes constant $S(a)$, which is \emph{algebraic} ($S(a)=S_0a^p$,
$S_0=1/\pi$; \S\ref{ssec:adep}) and hence bounded for $a\ge a_0$. That the off-diagonal keeps the
level action's Borel radius for \emph{every} $\beta$, not merely $\beta=2$, is Lemma~\ref{lem:rigidity}:
the location is $\beta$-independent, only the amplitude $S(a)$ carrying $\beta$ --- so the
uniform-in-$a$ off-diagonal here and the general-$\beta$ level inheritance of Theorem~\ref{thm:T1} are
one and the same statement. The two directions share the single
action $\Phi$; this alignment is why one Stokes constant governs the amplitude and the level and noise
singularities never cross. \tagPr
\end{remark}
...
orders. This is a theorem rather than a heuristic: Lemma~\ref{lem:rigidity} proves the rigidity from
the $\eps$-independence of the eikonal and leading transport of \eqref{eq:bk}, the distributional
potential playing no role (Remark~\ref{rem:rough}); $\beta$ moves only the Stokes matrices. \tagPr\
\emph{(both orders also confirmed numerically.)}
...
The reduced resolvent is available in closed form: constructing $G_{\mathrm{red}}$ from the Dirichlet
Airy Green's function and subtracting the pole at $\Lambda_0$, the diagonal reduces to two
Airy-quartic moments, and \eqref{eq:om1} becomes
\begin{equation}\label{eq:om1exact}
\Omega_1=\frac{4\pi}{\Ai'(-\Lambda_0)^2}\left[\frac{\Bi'(-\Lambda_0)}{\Ai'(-\Lambda_0)}
\int_{a_1}^\infty\!\Ai^4-\int_{a_1}^\infty\!\Ai^3\Bi\right],
\end{equation}
the third natural term $\int_{a_1}^\infty\Ai^3\Ai'=\tfrac14[\Ai^4]_{a_1}^\infty=0$ vanishing exactly
because $a_1=-\Lambda_0$ is a zero of $\Ai$. Evaluating \eqref{eq:om1exact} to high precision gives
\begin{equation}\label{eq:om1val}
\Omega_1=-1.124813904375434066\ldots,\qquad
\E[\Lambda_0]=2.3381074105-\tfrac{1.1248139}{\beta}+O(\beta^{-2}),
\end{equation}
consistent with the two earlier evaluations ($-1.121$ sum-over-states; $-1.128\pm0.009$
direct-diagonalization Monte Carlo with a Malliavin/Stein estimator \cite{Viens}). The quartic
moment $\int_{a_1}^\infty\Ai^4$ is not reducible to boundary terms (no antiderivative polynomial in
$x$ over the Airy monomials), so $\Omega_1$ is a genuine Airy-quartic transcendental---no elementary
closed form. Being real (self-adjointness) is the structural contrast with the cusp, whose
unbounded-below Weber operator has a complex resonance $\lambda_0=0.8896(1-i)$ and complex
$\Omega_2=-0.451+0.352i$. \tagPr
...
whereupon Costin \cite{Costin} closes summability; the $\beta$-independence of its Borel locations and
index --- hence the inheritance beyond the exact point $\beta=2$ --- is the noise-rigidity
Lemma~\ref{lem:rigidity}. \textbf{T2} --- location-fixing (a), the real shift
the closed-form connection-root shift $\Omega_1=-1.1248139\ldots$ (b, eq.~\eqref{eq:om1exact}, two
Airy-quartic moments, cross-checked by two independent evaluations), the exact Stokes constant
$C=\sqrt{2/(3\pi^3)}$ (c, $28$
...
integral (Lemma~\ref{lem:airy}, Prop.~\ref{prop:borel}), and \emph{uniformly in $a$} for $a\ge a_0\approx1$
(Rem.~\ref{rem:uniform}: sector on $\R_+$, the diagonal an odd/median part of the frozen escape, the
amplitude the algebraic Stokes constant $S(a)$). \emph{Cited (turnkey):}
...
The $\alpha=\tfrac12/\alpha=0$ family assignment is now settled by the saddle-vs-distribution tail
match (Rem.~\ref{rem:family}). The rough-potential rigidity previously flagged here is now proved
(Lemma~\ref{lem:rigidity}, Rem.~\ref{rem:rough}), so the general-$\beta$ level inheritance and the
uniform-in-$a$ off-diagonal both reduce to it. \emph{Refinements remaining:} the explicit large-order
value of $S(a)$'s subleading $a$-corrections (needs the all-order $R_m$). \emph{Open frontier:}
Conjecture~\ref{conj:frontier}.

\paragraph{Positioning.} Our contribution is \emph{not} the value of the Stokes constant
$C=\tfrac1\pi\sqrt{2/(3\pi)}$ --- that is the known Hastings--McLeod datum
\cite{Dunne2025,KapaevHM,FIKN}, re-derived here --- but three things: (i) the exact affine Lax map
(Lemma~\ref{lem:lax}) identifying the tail's fluctuation operator \emph{as} the Painlev\'e~II
linearization and the instanton as the $\alpha=\tfrac12$ Airy solution; (ii) the resulting
two-directional resurgent trans-series --- level (PII, Stokes constant $C=\sqrt{2/(3\pi^3)}$) and
noise ($1/\beta$, two sectors: dynamical escape and Coulomb-gas) --- for a \emph{random-operator}
edge law; and (iii) the Gelfand--Yaglom / collective-coordinate route to $C$ (Prop.~\ref{prop:gy}).
To our knowledge this is the first Borel-summable resurgent trans-series for a random-operator edge
law, and it sits apart from three neighbouring lines. \emph{Deterministic kernel-determinant
resurgence:} Painlev\'e~II resurgence in matrix models / minimal strings \cite{SchiappaVaz} treats
the string-equation solution, not Hastings--McLeod; the ``generalized Tracy--Widom'' resurgence of
the gauge-theory Bessel-kernel determinants \cite{BajnokTW,BajnokUniv} expands in the 't~Hooft
coupling, not the Dyson index, and concerns a deterministic Fredholm determinant rather than the
stochastic-Airy tail. \emph{Rate-function-only instantons:} the orthogonal-polynomial / Coulomb-gas
instanton analysis of extreme eigenvalues \cite{AtkinZohren} identifies the right-tail rate with an
instanton action but stops short of the fluctuation-determinant Stokes data. \emph{The large-$\beta$
reduction:} \cite{CLDS} reduce the rate function and $O(1/\beta)$ cumulants to PII with no resurgence
analysis; we supply the operator-level linearization and the Borel-summable trans-series built on it.
It is also the theorem-grade sibling of the cusp companion, where the absence of a Painlev\'e
reduction forces a numerical partial resurgent representation; the clean structural dichotomy
persists --- the cusp's complexity a \emph{resonance} (non-self-adjoint, unbounded-below) phenomenon,
$\TW$'s a \emph{trans-series} (self-adjoint, median-resummation) one --- and the one genuinely open
piece common to both is Conjecture~\ref{conj:frontier}: a direct exact-WKB of the random operator.
...
\bibitem{SchiappaVaz} R.~Schiappa, R.~Vaz, \emph{The resurgence of instantons: multi-cut Stokes
phases and the Painlev\'e~II equation}, Comm. Math. Phys. \textbf{330} (2014) 655--721;
arXiv:1302.5138.

\bibitem{AtkinZohren} M.~R.~Atkin, S.~Zohren, \emph{Instantons and extreme value statistics of
random matrices}, J. High Energy Phys. \textbf{2014} (2014) 118; arXiv:1307.3118.

\bibitem{BajnokTW} Z.~Bajnok, B.~Boldis, G.~P.~Korchemsky, \emph{Tracy--Widom distribution in
four-dimensional super-Yang--Mills theories}, arXiv:2403.13050 (2024).

\bibitem{BajnokUniv} Z.~Bajnok, B.~Boldis, D.~le~Plat, \emph{Universality in the resurgence of
generalized Tracy--Widom distributions}, arXiv:2509.20302 (2025).
