        ...
        for dI in args.deltaI:
            ...
            for eps in args.eps:
                ...
                for sigma in args.sigmas:
                    ...
                    # Early means: peel occurs at larger W than deterministic, or earlier time.
                    # W decreases along the local passage, so larger W = earlier departure.
                    early_by_W = (~np.isnan(Wp)) & np.isfinite(det_W) & (Wp > det_W + args.early_W_margin)
                    early_by_t = (~np.isnan(tp)) & np.isfinite(det_t) & (tp < det_t - args.early_t_margin)
