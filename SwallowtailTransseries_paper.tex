...
\emph{The identified route.} The most concrete way past (i) and (iii) is not more rungs but the
deterministic anchor of Rem.~\ref{rem:anchorexists}. Importing the cubic TBA/quantisation data
\cite{Voros1999,Masoero2010,ItoMarinoShu,AllegrettiBridgeland} would supply $(A,\theta)$
\emph{independently} of the ladder, which is precisely the input both refuted methods needed and
could not generate from within. That converts the circular obstruction into a finite piece of work:
compute the cubic Stokes data by TBA, then use it to prescribe the conformal map or the
non-perturbative subtraction. We flag this as the single highest-value continuation.
